...
# Fix macOS NLTK SSL certificate verification issue
try:
...
# Common Skills list for categorization
SKILLS_LIST = [
...
def analyze_skills(jd_text, resume_text):
    ...
    for skill in SKILLS_LIST:
        # Avoid partial word matches (e.g. 'go' matching inside 'google')
        pattern = r'\b' + re.escape(skill) + r'\b'
        if re.search(pattern, jd_clean):
            required_skills.append(skill)
            
    # Check if they are matched or missing in the resume
    matched = []
    missing = []
...
# Print any loading warnings in the sidebar
if errors:
...
# Default Job Description (ML Engineer)
...
if submit_btn:
    ...
    if not resume_text.strip():
        ...
        with st.spinner("Processing texts & running matching models..."):
            ...
            # Map score to labels and percentages
            labels_map = {
                1: "Very Poor Match",
                2: "Poor Match",
                3: "Fair Match",
        ...
        # Render CSS variable block for dynamic circular progress and colors
        score_styles = f"""
        <style>
        .score-circle {{
...
# Interactive tabs for static info (always visible at the bottom)
...
with tab_comp:
    ...
    # Setup comparison metrics dataframe
    comparison_data = {
